...
@Component(
	property = {
		"class.name=com.liferay.commerce.product.model.impl.CPOptionImpl",
		"table.name=CPOption"
	},
...
public class CPOptionModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(cpOptionModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		CPOptionModelImpl cpOptionModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = cpOptionModelImpl.getColumnOriginalValue(columnName);
			}
			else {
				value = cpOptionModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-1546940139