...
@Component(
	property = {
		"class.name=com.liferay.commerce.model.impl.CommerceShipmentImpl",
		"table.name=CommerceShipment"
	},
...
public class CommerceShipmentModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(commerceShipmentModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = commerceShipmentModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = commerceShipmentModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-408545172