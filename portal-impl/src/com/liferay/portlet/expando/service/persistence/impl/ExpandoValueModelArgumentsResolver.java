...
@OSGiBeanProperties(
	property = {
		"class.name=com.liferay.portlet.expando.model.impl.ExpandoValueImpl",
...
public class ExpandoValueModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(expandoValueModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		ExpandoValueModelImpl expandoValueModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = expandoValueModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = expandoValueModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:1317429081