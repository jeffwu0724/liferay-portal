...
public class DepotEntryPinModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(depotEntryPinModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		DepotEntryPinModelImpl depotEntryPinModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = depotEntryPinModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = depotEntryPinModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:243635968