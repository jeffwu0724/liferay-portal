...
public class ContactModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(contactModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		ContactModelImpl contactModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = contactModelImpl.getColumnOriginalValue(columnName);
			}
			else {
				value = contactModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:1599577285