...
@Component(
	property = {
		"class.name=com.liferay.change.tracking.model.impl.CTAutoResolutionInfoImpl",
		"table.name=CTAutoResolutionInfo"
	},
...
public class CTAutoResolutionInfoModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(
				ctAutoResolutionInfoModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = ctAutoResolutionInfoModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = ctAutoResolutionInfoModelImpl.getColumnValue(
					columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-445050464