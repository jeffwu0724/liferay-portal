...
public class CommerceTermEntryModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(commerceTermEntryModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = commerceTermEntryModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = commerceTermEntryModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:524362789