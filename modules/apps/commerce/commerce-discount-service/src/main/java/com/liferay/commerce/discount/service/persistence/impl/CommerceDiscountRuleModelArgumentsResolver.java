...
@Component(
	property = {
		"class.name=com.liferay.commerce.discount.model.impl.CommerceDiscountRuleImpl",
		"table.name=CommerceDiscountRule"
	},
...
public class CommerceDiscountRuleModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(
				commerceDiscountRuleModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = commerceDiscountRuleModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = commerceDiscountRuleModelImpl.getColumnValue(
					columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-1659609206