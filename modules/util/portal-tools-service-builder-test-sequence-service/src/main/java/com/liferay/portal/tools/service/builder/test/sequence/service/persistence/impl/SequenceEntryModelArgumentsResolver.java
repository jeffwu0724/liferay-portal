...
public class SequenceEntryModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(sequenceEntryModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		SequenceEntryModelImpl sequenceEntryModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = sequenceEntryModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = sequenceEntryModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:1876492781