...
/**
 * The arguments resolver class for retrieving value from AnnouncementsEntry.
 *
...
public class AnnouncementsEntryModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(announcementsEntryModelImpl, finderPath, original);
		}
		...
		if (finderPathColumnBitmask == null) {
			...
			for (String columnName : columnNames) {
				finderPathColumnBitmask |=
					announcementsEntryModelImpl.getColumnBitmask(columnName);
			}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = announcementsEntryModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = announcementsEntryModelImpl.getColumnValue(columnName);
			}
	...
	static {
		...
		orderByColumnsBitmask |= AnnouncementsEntryModelImpl.getColumnBitmask(
			"priority");
		orderByColumnsBitmask |= AnnouncementsEntryModelImpl.getColumnBitmask(
			"modifiedDate");
...
// LIFERAY-SERVICE-BUILDER-HASH:-1286515535