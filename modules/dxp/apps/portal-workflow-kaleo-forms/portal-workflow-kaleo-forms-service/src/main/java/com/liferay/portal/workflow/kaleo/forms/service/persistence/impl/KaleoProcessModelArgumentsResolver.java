...
public class KaleoProcessModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(kaleoProcessModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		KaleoProcessModelImpl kaleoProcessModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = kaleoProcessModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = kaleoProcessModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-713712608