...
@OSGiBeanProperties(
	property = {
		"class.name=com.liferay.portal.model.impl.AddressImpl",
		"table.name=Address"
	},
...
public class AddressModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(addressModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		AddressModelImpl addressModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = addressModelImpl.getColumnOriginalValue(columnName);
			}
			else {
				value = addressModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-316582926