...
public class CommercePriceModifierModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(
				commercePriceModifierModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = commercePriceModifierModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = commercePriceModifierModelImpl.getColumnValue(
					columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-1872551251