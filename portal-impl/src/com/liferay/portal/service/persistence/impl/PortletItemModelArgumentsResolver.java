...
public class PortletItemModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(portletItemModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		PortletItemModelImpl portletItemModelImpl, FinderPath finderPath,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = portletItemModelImpl.getColumnOriginalValue(columnName);
			}
			else {
				value = portletItemModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-2111005626