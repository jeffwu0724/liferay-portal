...
public class ClusterGeneralTest {
		...
		try {
			...
			_tomcatNode2.syncExecute(
				() -> {
					...
					// will update all its properties, even though
					// only one change in master node
			...
			Assert.assertEquals(
				...
				_tomcatNode2.syncExecute(
					() -> {
						_getTestPropertyChangeListenerCountDownLatch().await();
		...
		try {
			...
			_tomcatNode1.syncExecute(
				() -> {
					...
					// master node will only update the change
			...
			_tomcatNode2.syncExecute(
				() -> {
					...
					ServiceReference<ClusterMasterTokenTransitionListener>
						serviceReference = serviceReferences.get(
							serviceReferences.size() - 1);
	...
	private static CountDownLatch
		_getTestPropertyChangeListenerCountDownLatch() {

		LoggerContext loggerContext = LoggerContext.getContext();

		List<TestPropertyChangeListener> propertyChangeListeners =
			ReflectionTestUtil.getFieldValue(
				loggerContext, "propertyChangeListeners");

		TestPropertyChangeListener testPropertyChangeListener =
			propertyChangeListeners.get(0);

		return testPropertyChangeListener.getCountDownLatch();
	}
