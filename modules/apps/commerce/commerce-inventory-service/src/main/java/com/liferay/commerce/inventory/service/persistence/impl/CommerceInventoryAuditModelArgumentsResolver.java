...
/**
 * The arguments resolver class for retrieving value from CommerceInventoryAudit.
 *
...
@Component(
	property = {
		"class.name=com.liferay.commerce.inventory.model.impl.CommerceInventoryAuditImpl",
		"table.name=CIAudit"
	},
	service = ArgumentsResolver.class
)
public class CommerceInventoryAuditModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		long columnBitmask = commerceInventoryAuditModelImpl.getColumnBitmask();

		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(
				commerceInventoryAuditModelImpl, finderPath, original);
		}
		...
		if (finderPathColumnBitmask == null) {
			...
			if (finderPath.isBaseModelResult() &&
				(CommerceInventoryAuditPersistenceImpl.
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = commerceInventoryAuditModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
				value = commerceInventoryAuditModelImpl.getColumnValue(
					columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:193699416